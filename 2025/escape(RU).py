...
objects = {
    ...
    43: [images.whiteboard, images.full_shadow,
         "Белая доска используется при мозговых штурмах и совещаниях по планированию."],
    ...
    "Доска для записей. Кто-то что-то нацарапал на ней.", "доска для записей"],
    54: [images.bubble_gum, None,
    "Кусочек липкой жевательной резинки. Со вкусом космической ягоды.", "жевательная резинка"],
    55: [images.yoyo, None, "Игрушка из тонкой прочной верёвки и пластика. \n"
        "Используется для экспериментов с антигравитацией.", PLAYER_NAME + "йойо"],
    ...
    64: [images.rags, None,
    "Промасленная тряпка. Возьми его за один угол, если нужно!", "Промасленная тряпка"],
        65: [images.hammer, None,
    "Молоток. Может пригодиться для того, чтобы что-то расколоть..." "молоток"],
    66: [images.spoon, None, "Большая столовая ложка", "ложка"],
    67: [images.food_pouch, None,
    "Пакетик с обезвоженным кормом. Ему нужна вода", "сухой корм"],
    68: [images.food, None,
    "Пакетик с кормом. Используйте его, чтобы получить 100% энергии.", "готовая к употреблению еда"],
    69: [images.book, None, "На обложке книги большими дружелюбными буквами \
     написано 'Не паникуй'", "книга"],
    70: [images.mp3_player, None,
    "MP 3 -плеер со всеми последними мелодиями", "MP 3 -плеер"],
    ...
    72: [images.radio, None, "Система радиосвязи, доставленная \
    с Пудель", "радиостанция"],
...
чтобы что-то резать. Вы можете их заточить?", "тупые ножницы"],
    ...
    80: [images.access_card, None,
         "Эта карта доступа принадлежит " + FRIEND1_NAME, "карта доступа"],
    81: [images.access_card, None,
         "Эта карта доступа принадлежит " + FRIEND2_NAME, "карта доступа"]
    }
...
def start_room():
    ...
    show_text("Вы находитесь в комнате: " + room_name, 0)
...
def pick_up_object():
    ...
    # Очистить место на полу.
        ...
        show_text("Теперь вы несёте " + objects[item_player_is_on][3], 0)
        sounds.pickup.play()
        time.sleep(0.5)
    else:
        show_text("Вы не можете нести это!", 0)
...
def examine_object():
    ...
    if item_player_is_on in [0, 2]:  # не описывайте пол
        return
    description = "Вы видите: " + objects[item_player_is_on][2]
...
def use_object():
    ...
    use_message = "Вы возитесь с ним, но ничего не получается."
    ...
    standard_responses = {
        ...
        34: "Ах! Так-то лучше. Теперь вымойте руки.",
        35: "Вы моете руки и стряхиваете воду.",
        37: "Пробирки слегка дымятся, когда вы их встряхиваете.",
        54: "Вы жуёте жвачку. Она липкая, как клей.",
        55: "Йойо подпрыгивает вверх и вниз, чуть медленнее, чем на Земле.",
        56: "Это немного сложно. Вы можете привязать его к чему-нибудь?",
        59: "Вам нужно устранить утечку, прежде чем вы сможете использовать баллон",
        61: "Вы пытаетесь подать сигнал с помощью зеркала, но вас никто не видит",
        62: "Не тратьте ресурсы впустую. Вещи могут пригодиться...",
        67: "Чтобы насладиться вкусной космической едой, просто добавьте воды!",
        75: "Вы находитесь в секторе: " + str(current_room) + " // X: " \
    ...
    # Вернитесь, чтобы остановить использование другого объекта, случайно заменившего этот.
       ...
       and not suit_stitched:
        ...
        use_message = "Вы вызываете помощь по радио. Приближается спасательный корабль. \
        ...
        if energy > 100:
            energy = 100
        use_message = "Вы жуёте салат и получаете немного энергии обратно"
        draw_energy_air()
        ...
        use_message = "Вы нажимаете кнопку"
        ...
        energy = 100
        use_message = "Вы используете еду, чтобы восстановить энергию"
        remove_object(68)
        draw_energy_air()
